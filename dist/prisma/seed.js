...
const neonDb = new client_1.PrismaClient({
    datasources: {
        db: {
...
const localDb = new client_1.PrismaClient();
async function main() {
    ...
    const neonTipos = await neonDb.$queryRawUnsafe(`SELECT * FROM "Admin"."USUARIO_TIPO"`);
    ...
    const neonUsers = await neonDb.$queryRawUnsafe(`SELECT * FROM "Admin"."USUARIO" ORDER BY id_usuario`);
    ...
    const neonEmpresas = await neonDb.$queryRawUnsafe(`SELECT * FROM "Empresa"."EMPRESA" ORDER BY id_empresa`);
    let empOk = 0;
    for (const e of neonEmpresas) {
        const user = await localDb.usuario.findUnique({ where: { username: (await neonDb.$queryRawUnsafe(`SELECT username FROM "Admin"."USUARIO" WHERE id_usuario = ${e.id_usuario}`))[0]?.username } });
        ...
        const neonUserRow = await neonDb.$queryRawUnsafe(`SELECT username FROM "Admin"."USUARIO" WHERE id_usuario = ${e.id_usuario}`);
        if (!neonUserRow.length)
            continue;
    ...
    const neonEstudiantes = await neonDb.$queryRawUnsafe(`SELECT * FROM "Estudiante"."ESTUDIANTE" ORDER BY id_estudiante`);
    let estOk = 0;
    ...
    const neonFormacion = await neonDb.$queryRawUnsafe(`SELECT * FROM "Estudiante"."FORMACION_ACADEMICA"`);
    let formOk = 0;
    for (const f of neonFormacion) {
        const neonEst = await neonDb.$queryRawUnsafe(`SELECT e.*, u.username FROM "Estudiante"."ESTUDIANTE" e JOIN "Admin"."USUARIO" u ON u.id_usuario = e.id_usuario WHERE e.id_estudiante = ${f.id_estudiante}`);
        if (!neonEst.length)
            continue;
