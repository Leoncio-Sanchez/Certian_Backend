...
class AuthService {
    ...
    async registerUser(userData) {
        ...
        const existingUser = await prisma_1.prisma.usuario.findFirst({ where: { username } });
        if (existingUser) {
        ...
        if (id_usuario_tipo === 3) { // ESTUDIANTE
            ...
            const existingDni = await prisma_1.prisma.estudiante.findUnique({ where: { dni: finalDni } });
            if (existingDni)
                throw new Error('DNI already registered');
        }
            ...
            const existingRuc = await prisma_1.prisma.empresa.findUnique({ where: { ruc: finalRuc } });
            if (existingRuc)
        ...
        try {
            if (id_usuario_tipo === 3) { // ESTUDIANTE
                await prisma_1.prisma.estudiante.create({
                    data: {
                ...
                // Assign USER role
                await prisma_1.prisma.usuarioRol.create({
                    data: {
                ...
                await prisma_1.prisma.empresa.create({
                    data: {
                        id_usuario: user.id_usuario,
            ...
            await prisma_1.prisma.usuario.delete({ where: { id_usuario: user.id_usuario } });
            throw profileError;
        }
